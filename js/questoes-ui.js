...
/**
 * Mostra a resposta correta + resolução comentada — só aparece quando o backend manda
 * `gabarito` junto (o que só acontece depois que o aluno já respondeu E o professor liberou
 * a resolução; ver buscarQuestoesParaResponder no backend). Em modo "responder" normal,
 * q.gabarito é undefined e esta função não renderiza nada.
 */
function renderResolucaoQuestao(q) {
  if (q.gabarito === undefined) return '';
  let respostaCorreta = '';
  switch (q.tipo) {
    case 'multipla':
      respostaCorreta = `<strong>Resposta correta:</strong> ${escapeHtml(q.gabarito)}) ${formatarTextoQuestao((q.alternativas || {})[q.gabarito] || '')}`;
      break;
    case 'vf':
      respostaCorreta = '<strong>Respostas corretas:</strong><br>' + (q.alternativas || []).map(af =>
        `${formatarTextoQuestao(af.texto)} — <em>${(q.gabarito || {})[af.id] ? 'Verdadeiro' : 'Falso'}</em>`).join('<br>');
      break;
    case 'relacione': {
      const colB = (q.alternativas && q.alternativas.colunaB) || [];
      respostaCorreta = '<strong>Correspondência correta:</strong><br>' + ((q.alternativas && q.alternativas.colunaA) || []).map(a => {
        const itemB = colB.find(b => b.id === (q.gabarito || {})[a.id]);
        return `${formatarTextoQuestao(a.texto)} → ${itemB ? formatarTextoQuestao(itemB.texto) : '?'}`;
      }).join('<br>');
      break;
    }
    case 'classifique':
    case 'ordenar': {
      ...
      respostaCorreta = '<strong>Ordem correta:</strong><ol>' + (q.gabarito || []).map(id => {
        const item = itens.find(i => i.id === id);
        return `<li>${item ? formatarTextoQuestao(item.texto) : '?'}</li>`;
      }).join('') + '</ol>';
      break;
    }
    case 'lacunas':
