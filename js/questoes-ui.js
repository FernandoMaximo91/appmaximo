  ...
  switch (tipo) {
    case 'multipla': {
      ...
      return ['A', 'B', 'C', 'D', 'E'].map(l => `
        <label>Alternativa ${l}${l === 'A' ? ' <span style="font-weight:400;">(marque a correta ao lado)</span>' : ''}</label>
        <div style="display:flex;gap:8px;align-items:center;">
          <input type="radio" name="gabarito-multipla" value="${l}" ${dados.gabarito === l ? 'checked' : ''}>
          <input type="text" id="edit-alt-${l}" value="${escapeHtml(alt[l] || '')}" style="flex:1;">
        </div>`).join('');
    }
    case 'vf': {
